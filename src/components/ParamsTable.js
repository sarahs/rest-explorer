...
const ParamsTable = ({method, paramsArray, enteredBodyParams, onChange, onClearBody, isBodyComplete}) => (
  <div>
    {isBodyComplete
    ...
    <table>
      ...
      <tbody>
          ...
          <tr key={i}>
            ...
            {typeof(p["required"]) !== 'undefined' ? <td><code>{p["required"].toString()}</code></td> : <td><code>N/A</code></td> }
            {p["default"] ? <td><code>{p["default"].toString()}</code></td> : <td><code>N/A</code></td> }
            <td><Markdown source={p["description"]}/></td>
            <ParamInput
              name={p["name"]}
              value={enteredBodyParams[p["name"]] ? enteredBodyParams[p["name"]] : ''}
              onChange={onChange}
            />
          </tr>
        )}
